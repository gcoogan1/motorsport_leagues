...
export const IconWrapper = styled.span<{ $hasValue?: boolean }>`
  ...
  pointer-events: none;
  display: flex;
  color: ${({ $hasValue }) => ($hasValue ? colors.text.text1 : colors.text.text2)};
  width: 20px;
  height: 20px;
`;
...
export const InputField = styled.input<{ $hasError: boolean; $hasValue: boolean }>`
  width: 100%;
  ...
  color: ${({ $hasValue }) => ($hasValue ? colors.text.text1 : colors.text.text2)};
  box-shadow: ${({ $hasError }) =>
    $hasError ? `inset 0 0 0 2px ${colors.alert.alertA}` : `inset 0 0 0 2px transparent`};
